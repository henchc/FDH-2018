from syllabipymhg3 import syllabipymhg
...
def get_features(string_text):

    lines = string_text.split('\n')
    lines_sylls = [[syllabipymhg(w) for w in line.split()] for line in lines]
    lines_sylls = [x for x in lines_sylls if len(x) > 0]
    ...
    return (text_with_features,
            [[x for i in l for x in i] for l in lines_sylls])
